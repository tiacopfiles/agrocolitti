...
try {
    ...
    if ($produto_id <= 0 || $kg_caixa <= 0) {
        if ($produto_id > 0) {
            salvarDisponibilidadePrecoTabela($conexao, 'preco_atacado', $produto_id, $ativo);
    ...
    foreach ($novasColunas as $col => $def) {
        if (!colunaExiste($conexao, 'preco_atacado', $col)) {
            $conexao->query("ALTER TABLE preco_atacado ADD COLUMN $col $def");
        }
    }
