...
$entreposto = comissoesFreteNormalizarEntreposto((string) ($_GET['entreposto'] ?? ''));
if ($entreposto === null) {
...
$stmt = $conexao->prepare("
    SELECT cf.id, cf.numero_os, cf.venda_id, cf.contabilizado_em,
           cf.valor_liquido, n.numero_nfe, n.emitida_em, n.autorizada_em,
           (
               SELECT v.status
               FROM vendas v
               WHERE CONVERT(v.numero_os USING utf8mb4) COLLATE utf8mb4_unicode_ci = cf.numero_os COLLATE utf8mb4_unicode_ci
               ORDER BY v.id DESC
               LIMIT 1
           ) AS venda_status
    ...
    INNER JOIN nfe_documentos n
        ON n.id = cf.nfe_documento_id
       AND n.tipo_emissao = 'venda'
       AND n.status = 'autorizada'
    WHERE cf.entreposto = ?
    ORDER BY cf.contabilizado_em DESC, cf.id DESC
");
$stmt->bind_param('s', $entreposto);
$stmt->execute();
$designacoes = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();

$subtotalFrete = array_sum(array_map(
    static fn(array $item): float => (float) ($item['valor_liquido'] ?? 0),
    $designacoes
));

function comissoesFreteData(?string $data): string
{
    if (!$data) return '-';
    $ts = strtotime($data);
...
<main class="container page-container app-shell">
  <div class="page-head">
    <div><h1><?= htmlspecialchars($entreposto, ENT_QUOTES, 'UTF-8') ?></h1><p>Pedidos designados para este entreposto.</p></div>
    <a class="btn" href="frete.php"><i class="bi bi-arrow-left"></i> Voltar aos entrepostos</a>
  </div>
  <section class="card table-wrap">
    <table>
      <thead><tr><th>Numero da nota</th><th>Data da emissao</th><th>Numero da OS</th><th>Total do pedido</th><th>Status da venda</th></tr></thead>
      <tbody>
      <?php if (!$designacoes): ?>
        <tr><td class="empty" colspan="5">Nenhuma venda nova designada para este entreposto.</td></tr>
      <?php else: foreach ($designacoes as $item):
      ...
      ?>
        <tr>
          ...
          <td><?= comissoesFreteData($dataNota) ?></td>
          <td class="os"><?= htmlspecialchars($item['numero_os']) ?></td>
          <td class="money">R$ <?= number_format((float) $item['valor_liquido'], 2, ',', '.') ?></td>
          <td><span class="status status-<?= htmlspecialchars($status) ?>"><?= htmlspecialchars(comissoesFreteStatusLabel($status)) ?></span></td>
        </tr>
      ...
      <tfoot>
        <tr>
          ...
          <td class="money">R$ <?= number_format($subtotalFrete, 2, ',', '.') ?></td>
          <td></td>
        </tr>
      </tfoot>
    </table>
